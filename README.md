...
When an external browser agent connects, the editor displays a live **WebMCP Status Pill** (`● WebMCP 16 tools`) and a real-time **Agent Activity HUD** showing each tool call as the agent builds the world collaboratively with the human developer.
